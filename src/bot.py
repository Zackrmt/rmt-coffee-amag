...
class TelegramBot:
    ...
    async def restore_interrupted_sessions(self):
        """Restore any interrupted sessions from persistence storage."""
        try:
            if self.application and hasattr(self.application, 'persistence'):
                # Get all user data from persistence
                all_user_data = await self.application.persistence.get_user_data()
                
                # Check each user for interrupted sessions
                for user_id, user_data in all_user_data.items():
                    if 'interrupted_session' in user_data:
                        saved_session = user_data['interrupted_session']
                        
                        # Create a new StudySession object
                        session = StudySession(
                            user_id=saved_session['user_id'],
                            subject=saved_session['subject'],
                            goal_time=saved_session.get('goal_time')
                        )
                        
                        # Restore session properties
                        session.start_time = saved_session['start_time']
                        session.break_periods = saved_session.get('break_periods', [])
                        
                        # If session was on break when interrupted, restore break state
                        if saved_session.get('is_on_break') and saved_session.get('break_start'):
                            session.is_on_break = True
                            session.break_start = saved_session['break_start']
                        
                        # Add the restored session to active sessions
                        self.study_sessions[user_id] = session
                        
                        logger.info(f"Restored interrupted session for user {user_id}")
                        
                logger.info(f"Restored {len(self.study_sessions)} interrupted sessions")
        except Exception as e:
            logger.error(f"Error restoring interrupted sessions: {e}")

    async def recover_buttons(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle callback queries from buttons that were created before redeployment."""
        self.record_activity()
        query = update.callback_query
        user_id = update.effective_user.id
        
        # Log the callback data for debugging
        logger.info(f"Received callback query: {query.data} from user {user_id}")
        
        # First, try to answer the callback to prevent the loading indicator
        try:
            await query.answer()
        except Exception as e:
        ...
        if query.data == 'start_break':
            # Handle the start break button
            return await self.handle_break(update, context)
        elif query.data == 'end_break':
            # Handle the end break button
            return await self.handle_break(update, context)
        elif query.data == 'end_session':
            # Handle the end session button
            return await self.end_session(update, context)
        elif query.data == 'start_studying':
            # Handle the start studying button
            return await self.ask_goal(update, context)
        elif query.data == 'overall_progress':
            # Handle the overall progress button
            return await self.generate_overall_progress_report(update, context)
        elif query.data == 'today_report':
            # Handle the today report button
            return await self.generate_today_report(update, context)
        elif query.data == 'report_session':
            # Handle the session report button
            return await self.generate_session_report(update, context)
        elif query.data == 'report_day':
            # Handle the day report button
            return await self.generate_day_report(update, context)
        elif query.data == 'report_overall':
            # Handle the overall report button
            return await self.generate_overall_progress_report(update, context)
        elif query.data.startswith('goal_'):
            # Handle goal selection buttons
            return await self.handle_goal_selection(update, context)
        elif query.data == 'no_goal':
            # Handle no goal button
            return await self.handle_goal_selection(update, context)
        elif query.data.startswith('subject_'):
            # Handle subject selection buttons
            return await self.start_studying(update, context)
            ...
            return await self.handle_cancel_confirmation(update, context)
        elif query.data == 'reject_cancel':
            # Handle reject cancel button
            return await self.handle_cancel_confirmation(update, context)
        elif query.data == 'confirm_reset_data':
            # Handle confirm reset data button
            return await self.handle_reset_confirmation(update, context)
        elif query.data == 'cancel_reset_data':
            # Handle cancel reset data button
            return await self.handle_reset_confirmation(update, context)
        else:
            # Unknown button
            await query.message.reply_text(f"Button action not recognized. Please try a different action or start a new session.")
            return CHOOSING_MAIN_MENU

    async def end_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /END command to end study sessions, with optional time parameter."""
        self.record_activity()
        ...
        # Check for time parameter in the command
        command_text = update.message.text.strip()
        manual_end_time = None
        
        # Check for time parameter format like /END0330AM or /END1530
        time_pattern = r'\/END(\d{3,4}(?:AM|PM)?)'
        time_match = re.search(time_pattern, command_text, re.IGNORECASE)
        
        if time_match:
            manual_end_time = self._parse_end_time(time_match.group(1))
        
        # Try to retrieve active session
        session = self.study_sessions.get(user_id)
        
        # If no active session found, check if there might be a pending session that was interrupted
        if not session:
            # Check persistence for user data that might have an interrupted session
            ...
            if 'interrupted_session' in user_data:
                ...
                await self.send_bot_message(
                    ...
                    "🔄 Found an interrupted study session. Resuming from where you left off.",
                    should_delete=True
                )
        ...
        # If manual end time was provided, set it
        if manual_end_time:
            ...
            session.end_time = manual_end_time
            
            await self.send_bot_message(
                context,
                update.effective_chat.id,
                f"⏱️ Session end time manually set to {manual_end_time.astimezone(MANILA_TZ).strftime('%I:%M %p')}",
                should_delete=True
            )
        else:
        ...
        # Process the ended session
        return await self._process_ended_session(update, context, session)
    
    async def end_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle 'END' text messages to end study sessions, with optional time parameter."""
        self.record_activity()
        message_text = update.message.text.strip().upper()
        
        # Only process messages that start with END
        if not message_text.startswith("END"):
            return
        ...
        # Check for time parameter in the message
        manual_end_time = None
    ...
    async def _process_ended_session(self, update: Update, context: ContextTypes.DEFAULT_TYPE, session):
        """Process a session that has been ended, generating reports and cleaning up."""
        user = update.effective_user
        user_id = user.id
        
        # Get Manila timezone formatted times for display
        ...
        try:
            ...
            # Clear any interrupted session data
            if 'interrupted_session' in context.user_data:
                del context.user_data['interrupted_session']
            ...
            logger.error(f"Error in _process_ended_session: {e}")
            await self.send_bot_message(
                context,
                update.effective_chat.id,
    ...
    def _parse_end_time(self, time_str):
        """Parse a time string like '0330AM' or '1530' into a datetime object."""
        try:
            current_time = datetime.datetime.now(PST_TZ)
            ...
            # Determine if AM/PM is specified
            has_am_pm = "AM" in time_str or "PM" in time_str
            
            # Extract the numeric part
            numeric_part = re.sub(r'[^\d]', '', time_str)
            
            # Format depends on the length and whether AM/PM is specified
            if len(numeric_part) == 3:
                # Format like 330 (3:30)
                hour = int(numeric_part[0])
                minute = int(numeric_part[1:3])
            elif len(numeric_part) == 4:
                # Format like 0330 or 1530
                hour = int(numeric_part[0:2])
                minute = int(numeric_part[2:4])
            else:
            ...
            if has_am_pm:
                if "PM" in time_str and hour < 12:
                    ...
                    hour = 0
            elif hour < 12 and current_time.hour >= 12:
                # If no AM/PM specified and current time is PM, assume the time is also PM
                hour += 12
                
            # Create datetime with PST timezone
            end_time = datetime.datetime.combine(current_date, datetime.time(hour, minute))
            end_time = PST_TZ.localize(end_time)
            
            # If the specified time is in the future, assume it's for the previous day
    ...
    # Add this method to save session state during a study session
    async def save_session_state(self, user_id, context):
        """Save current session state to make it resilient to redeployments."""
        try:
            session = self.study_sessions.get(user_id)
            
            if session:
                # Get the user data from context using user_id
                ...
                user_data['interrupted_session'] = serialized_session
    ...
    # Modify the existing end_session method to reuse the _process_ended_session method
...
async def run_bot_with_retries():
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            application.add_handler(CommandHandler('END', telegram_bot.end_command), group=-1)
            ...
            # After adding all handlers, but before initializing the application
            application.add_handler(conv_handler)
            application.add_error_handler(error_handler)
            
            # Add this job queue code here
            # Add persistence callbacks to save session state periodically
            # This will help recover from redeployments
            application.job_queue.run_repeating(
                lambda context: asyncio.gather(*[
                    telegram_bot.save_session_state(user_id, context)
                    for user_id in telegram_bot.study_sessions
                ]), 
            ...
            # Add this line RIGHT HERE, between start() and start_polling()
            await telegram_bot.restore_interrupted_sessions()
            
            # Then start polling
            await application.updater.start_polling(
                poll_interval=3,
                timeout=30,
                drop_pending_updates=False,  # Changed to FALSE to process messages sent while down
                read_timeout=30,
                write_timeout=30
            )
            ...
            # Replace the existing polling code with this
            await application.updater.start_polling(
                poll_interval=3,
                timeout=30,
                drop_pending_updates=False,  # CHANGED TO FALSE to process messages sent while down
